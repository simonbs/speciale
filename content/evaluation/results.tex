...
\begin{itemize}
\item The system was only tested with users who had never tried to use the system before. One could imagine that users who have more experience with the system can train and perform gestures with a higher accuracy, thus potentially triggering the correct actions more often.
\item There is a risk that the gestures chosen for the test are too similar and thus the gesture recognizer produce are unable to precisely determine the gesture the user performed.
\item As part of the gesture recognition, a gesture trace is resampled in other to have the same length as the templates. We use a resampling rate of 64 to be consistent with 1\textcent \cite{herold20121}. \$3 use a resampling rate of 150 \cite{threedollar}. As our solution is a combination of both the 1\textcent and the \$3 gestues, it could be that a higher resampling rate would have resulted in a higher accuracy.
\end{itemize}
...
In \Cref{sec:evaluation:alternative-models} alternative models and improvements to the current model is presented.
