...
\Cref{fig:implementation:prototype:navigation-diagram} shows a navigation diagram for the user interface of the smartwatch application. The letters in the top right corner of the nodes in the diagram references the screenshots shown in \Cref{fig:implementation:prototype:screenshots}.
...
For a list of the supported features in the application, refer to \Cref{sec:implementation:status}.
...
The rooms and beacons are synchronized to the smartwatch when the application is loaded. The watch use the models when positioning the user.
...
%     }
%     \subbottom[Screen on which a new room can be created. The room should have a name, e.g. ``Kitchen'' or ``Living room''.]{\label{fig:implementation:prototype:openhab:room}
