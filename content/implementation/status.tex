...
We have not implemented any features on the BLE beacons but in order to position te user, the smart watch reads the RSSI values from the Bluetooth beacons and based on those values determines which room the user is in, described in detail in \Cref{sec:design:bayesian-network:room-node-evidence}.
...
As shown in \Cref{fig:implementation:deployment-diagram} we use HTTP for communication between the smart watch, the Raspberry Pi and the computer acting as media centre.
