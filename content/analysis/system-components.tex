...
We envision two different methods for positioning of the system. Both metods assume that bluetooth technology is utilized to determine the position of the user. \todo[author=Simon]{Reference to where we determine that bluetooth is used for positioning.}
...
\begin{itemize}
\item A Bluetooth LE peripheral (e.g. a wearable) can only be the slave of one master, that is, it can only be paired with a single other device.
...
The above lists the bare minimum of hardware required in order to implement the system. \cref{} provides a detailed analyses of the desired hardware. \todo[author=Simon]{Insert reference to detailed analyses of the hardware required for the system.}
